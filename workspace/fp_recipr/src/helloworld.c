...
int main(void)
{
    ...
    if (Error) {
         ...
         */
        Status = CheckData(MAX_PKT_LEN / 4 * NUMBER_OF_BDS_TO_TRANSFER,
                                    0xC);
        if (Status != XST_SUCCESS) {
    ...
    XTime_GetTime(&stop);
    uint32_t time = (((u64) stop - (u64) start) * 3) / 1000;
    xil_printf("Time: %u\r\n", time);
...
static int SendPacket(XAxiDma * AxiDmaInstPtr)
{
    ...
    Value = 1.0;
    for(Index = 0; Index < MAX_PKT_LEN / 4 * NUMBER_OF_BDS_TO_TRANSFER;
            Index ++) {
