...
int alarm_detect(u32 i2sAddr, u32 uartAddr)
{
    ...
    while (userInput != 'q')
    {
        ...
        {
            /* Store the first character in the UART receive FIFO */
            userInput = (unsigned int)XUartPs_ReadReg(uartAddr, XUARTPS_FIFO_OFFSET);
        }

        for (index = 1; index < 2*BUFF_LENGTH ; index++)
        {
            OutBuf[index] = (unsigned int)((int)I2SFifoRead(i2sAddr) >> 16);
            //I2SFifoWrite(i2sAddr, OutBuf[index]);
